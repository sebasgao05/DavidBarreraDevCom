...
interface SystemProps {
  ...
  handleSystemLoad: (category: string) => void;
}
...
const System: React.FC<SystemProps> = React.memo(({ category, index, isVisible, loadedSystems, handleSystemLoad }) => {
  ...
  }, [isLoaded, category, index, isVisible, handleSystemLoad]);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 min-h-[320px]">
          {currentItems.map((category, index) => (
            <System
              ...
              handleSystemLoad={handleSystemLoad}
            />
          ))}
        </div>
